...
/**
 * @type {Map<Number,String>} ( fileDataID, fileName ) 룩업테이블
 */
 ...
 * @param {String} buildConfig buildKey
 * @param {BuildCache} cache
 * @param {Map} rootEntries
 * @return {Number} count of fileDataIDs
 */
const loadListfile = async ( buildConfig, cache, rootEntries ) => 
{
    ...
    let url = constants.LISTFILE.listfileURL;
    ...
    // https://www.kruithne.net/wow.export/data/listfile/master?v=%s
    if ( url.includes( "%s" ) ) 
    {
    ...
    if ( url.startsWith( "http" ) ) 
    {
        ...
        if ( cache.meta.lastListfileUpdate ) 
        {
            ...
            let ttl = Number( constants.LISTFILE.listfileCacheRefresh ) || 0 ;
            ...
            if ( ttl === 0 || Date.now() - cache.meta.lastListfileUpdate > ttl ) 
            {
                ...
                log.write(
                    "Cached listfile for %s is out-of-date (> %d).",
                    buildConfig,
                    ttl
                );
            ...
            else 
            {
                ...
                if (cached === null) {
                    log.write(
                        "Listfile for %s is missing despite meta entry. User tamper?",
                        buildConfig
                    );
                    requireDownload = true;
                } else {
                    log.write(
                        "Listfile for %s is cached locally.",
                        buildConfig
                    );
                }
            }
        ...
        if ( requireDownload ) 
        {
            ...
            catch 
            {
                if ( cached === null )
                    throw new Error(
                        "Failed to download listfile, no cached version for fallback"
                    );

                log.write(
                    "Failed to download listfile, using cached as redundancy."
                );
                data = cached;
            }
        } 
...
const loadUnknowns = async () => {
    const unkBlp = await loadIDTable(
        DBTextureFileData.getFileDataIDs(),
        ".blp"
    );
    const unkM2 = await loadIDTable(DBModelFileData.getFileDataIDs(), ".m2");

    log.write(
        "Added %d unknown BLP textures from TextureFileData to listfile",
        unkBlp
    );
    log.write(
        "Added %d unknown M2 models from ModelFileData to listfile",
        unkM2
    );
    ...
    log.write(
        "Added %d unknown sound files from SoundKitEntry to listfile",
        unknownCount
    );
};
 ...
 * @param {Set<Number>} ids fileDataIDs
 * @param {String} ext
 */
...
const getFilenamesByExtension = ( exts ) => 
{
    ...
    /**
     * @type {Array<Number>} array of fileDataIDs
     */
    let entries = [];
 ...
 * @param {Array<Number>} entries array of fileDataIDs
 * @returns {Array<String>}
 */
const formatEntries = ( entries, sortByID=true, showID=true ) => 
{
    // If sorting by ID, perform the sort while the array is only IDs.
    if ( sortByID /*core.view.config.listfileSortByID*/ ) 
    {
        entries.sort( ( a, b ) => a - b );
    }

    if ( showID /*core.view.config.listfileShowFileDataIDs*/ )
    {
...
/**
 * 
 * @param {Map<Number,String>} entries ( fileDataID, ext ) 
 */
 ...
 * @param {Number} id fileDataID
 * @returns {String|Undefined} File Name ( Asset Name ) | Undefined
 */
const getByID = ( id ) => 
{
 ...
 * @param {Number} id fileDataID
 * @returns {String} file name ( asset name )
 */
...
const getByFilename = (filename) => {
    ...
    if (!lookup && (filename.endsWith(".mdl") || filename.endsWith("mdx")))
        lookup = nameLookup.get(
            ExportHelper.replaceExtension(filename, ".m2").replace(/\\/g, "/")
        );
 ...
 * @param {String} [ext] .xxx 
 * @return {String} unknown asset name ( unknown/fileDataID.ext )
 */
