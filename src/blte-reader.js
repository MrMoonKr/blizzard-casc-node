...
class BLTEIntegrityError extends Error {
    constructor(expected, actual) {
        super(
            util.format(
                "[BLTE] Invalid block data hash. Expected %s, got %s!",
                expected,
                actual
            )
        );
...
class BLTEReader extends BufferWrapper {
    ...
    _decryptBlock(data, blockEnd, index) {
        ...
        if (data.remainingBytes === 0)
            throw new Error(
                "[BLTE] Unexpected end of data before encryption flag."
            );

        const encryptType = data.readUInt8();
        if (encryptType !== ENC_TYPE_SALSA20)
            throw new Error(
                "[BLTE] Unexpected encryption type: " + encryptType
            );
