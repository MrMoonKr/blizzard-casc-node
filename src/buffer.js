...
class BufferWrapper {
    ...
    static async fromCanvas(canvas, mimeType) {
        ...
        else blob = await new Promise((res) => canvas.toBlob(res, mimeType));
    ...
    seek(ofs) {
        ...
        if (pos < 0 || pos > this.byteLength)
            throw new Error(
                util.format(
                    "seek() offset out of bounds %d -> %d ! %d",
                    ofs,
                    pos,
                    this.byteLength
                )
    ...
    move(ofs) {
        ...
        if (pos < 0 || pos > this.byteLength)
            throw new Error(
                util.format(
                    "move() offset out of bounds %d -> %d ! %d",
                    ofs,
                    pos,
                    this.byteLength
                )
    ...
    readBuffer(length = this.remainingBytes, wrap = true, inflate = false) {
        ...
        if (inflate) buf = zlib.inflateSync(buf);
    ...
    writeBuffer(buf, copyLength = 0) {
        ...
        if (buf instanceof BufferWrapper) {
            ...
            if (copyLength === 0) copyLength = buf.remainingBytes;
            else buf._checkBounds(copyLength);

            rawBuf = buf.raw;
        } else {
            if (copyLength === 0) copyLength = buf.byteLength;
        ...
        if (buf instanceof BufferWrapper) buf._ofs += copyLength;
    }
    ...
    indexOfChar(char, start = this.offset) {
        if (char.length > 1)
            throw new Error(
                "BufferWrapper.indexOfChar() given string, expected single character."
            );
