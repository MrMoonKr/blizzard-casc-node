...
class CASC {
    ...
    constructor( isRemote = false ) 
    {
        ...
        /**
         * @type {Map<String,String>} ( ContentKey, EncodeKey ) 룩업테이블
         */
        ...
        for ( const [fileDataID, entry] of this.rootEntries.entries() ) 
        {
            ...
            for ( const rootTypeIdx of entry.keys() ) 
            {
                ...
                if ( ( rootType.localeFlags & this.locale ) &&
                     ( rootType.contentFlags & ContentFlag.LowViolence ) === 0
                ) 
                {
                    include = true;
                    break;
    ...
    async getInstallManifest() {
        ...
        const installKey =
            installKeys.length === 1
                ? this.encodingKeys.get(installKeys[0])
                : installKeys[1];

        const raw = this.isRemote
            ? await this.getDataFile(this.formatCDNKey(installKey))
            : await this.getDataFileWithRemoteFallback(installKey);
    ...
    getModelFormats() {
        ...
        const modelExt = [];
        if (core.view.config.modelsShowM2) modelExt.push(".m2");
    ...
    updateListfileFilters() {
        ...
        core.view.listfileSounds = listfile.getFilenamesByExtension([
            ".ogg",
            ".mp3",
            ".unk_sound",
        ]);
        core.view.listfileVideos = listfile.getFilenamesByExtension(".avi");
        core.view.listfileText = listfile.getFilenamesByExtension([
            ".txt",
            ".lua",
            ".xml",
            ".sbt",
        ...
        core.view.listfileModels = listfile.getFilenamesByExtension(
            this.getModelFormats()
        );
    ...
    async loadTables() {
        ...
        if (core.view.config.enableM2Skins) {
            ...
            const creatureModelData = new WDCReader(
        ...
        if ( magic == ROOT_MAGIC ) 
        {
            ...
            while ( root.remainingBytes > 0 ) 
            {
                ...
                if (
                    !(
                        allowNamelessFiles &&
                        contentFlags & ContentFlag.NoNameHash
                    )
                )
