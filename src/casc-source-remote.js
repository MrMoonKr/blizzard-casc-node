...
class CASCRemote extends CASC {
        ...
        contentKey = null ) 
    {
        ...
        if ( data === null ) {
            ...
            if ( archive !== undefined ) 
            {
                ...
                if (!suppressLog)
                    log.write(
                        "Downloading CASC file %d from archive %s",
                        fileDataID,
                        archive.key
                    );
            } else {
                ...
                if (!suppressLog)
                    log.write(
                        "Downloading unarchived CASC file %d",
                        fileDataID
                    );

                if (data === null)
                    throw new Error(
                        "No remote unarchived/archive indexed for encoding key: " +
                            encodingKey
                    );
            }
    ...
    getProductList() {
        ...
        for (const entry of this.builds) {
            ...
            products.push(
                util.format("%s %s", product.title, entry.VersionsName)
            );
        }
