...
const initializeModelFileData = async () => {
    ...
    for (const [
        ...
        if (modelResIDToFileDataID.has(modelFileDataRow.ModelResourcesID))
            modelResIDToFileDataID
                .get(modelFileDataRow.ModelResourcesID)
                .push(modelFileDataID);
        else
            modelResIDToFileDataID.set(modelFileDataRow.ModelResourcesID, [
                modelFileDataID,
            ]);
    }
    log.write(
        "Loaded model mapping for %d models",
        modelResIDToFileDataID.size
    );
};
