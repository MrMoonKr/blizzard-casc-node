...
const convertDBDToSchemaType = (entry) => {
    ...
    if (entry.type === "int") {
        switch (entry.size) {
            case 8:
                return entry.isSigned ? FieldType.Int8 : FieldType.UInt8;
            case 16:
                return entry.isSigned ? FieldType.Int16 : FieldType.UInt16;
            case 32:
                return entry.isSigned ? FieldType.Int32 : FieldType.UInt32;
            case 64:
                return entry.isSigned ? FieldType.Int64 : FieldType.UInt64;
            default:
                throw new Error("Unsupported DBD integer size " + entry.size);
        }
    }
...
class WDCReader {
     ...
     * @param {string} fileName asset file name (ex) "DBFilesClient/ModelFileData.db2"
     */
    ...
    getRow(recordID) {
        ...
        if (!this.isLoaded)
            throw new Error(
                "Attempted to read a data table row before table was loaded."
            );
    ...
    getAllRows() {
        ...
        if (!this.isLoaded)
            throw new Error(
                "Attempted to read a data table rows before table was loaded."
            );
    ...
    async loadSchema(layoutHash) {
        ...
        const tableName = ExportHelper.replaceExtension(
        ...
        let structure = null;
        log.write(
            "Loading table definitions %s (%s %s)...",
            dbdName,
            buildID,
            layoutHash
        );
        ...
        if (structure === null) {
            try {
                ...
                await casc.cache.storeFile(
                    dbdName,
                    rawDbd,
                    constants.CACHE.DIR_DBD
                );

                // Parse the updated DBD and check for definition.
                structure = new DBDParser(rawDbd).getStructure(
                    buildID,
                    layoutHash
                );
            } catch (e) {
    ...
    getIDIndex() {
        if (!this.isLoaded)
            throw new Error(
                "Attempted to get ID index before table was loaded."
            );
    ...
    async parse( data ) 
    {
        ...
        if ( !data )
        {
            data = await core.view.casc.getFileByName(
                this.fileName,
                true,
                false,
                true
        ...
        for (let i = 0; i < totalFieldCount; i++)
            fields[i] = {
                size: data.readInt16LE(),
                position: data.readUInt16LE(),
            };
        ...
        for (
            let fieldIndex = 0, nFields = fieldInfo.length;
            fieldIndex < nFields;
            fieldIndex++
        ) {
            const thisFieldInfo = fieldInfo[fieldIndex];
            if (
                thisFieldInfo.fieldCompression ===
                    CompressionType.BitpackedIndexed ||
                thisFieldInfo.fieldCompression ===
                    CompressionType.BitpackedIndexedArray
            ) {
        ...
        assert.strictEqual(
            data.offset,
            prevPos + palletDataSize,
            "Read incorrect amount of pallet data"
        );
        ...
        assert.strictEqual(
            data.offset,
            prevPos + commonDataSize,
            "Read incorrect amount of common data"
        );
        ...
        for (
            ...
            const recordsOfs =
                wdcVersion === 2
                    ...
                    : header.offsetRecordsEnd;
            const recordDataSize = isNormal
                ? recordSize * header.recordCount
                : recordsOfs - header.fileOffset;
            ...
            const copyTableCount =
                wdcVersion === 2
                    ? header.copyTableSize / 8
                    : header.copyTableCount;
            ...
            // TODO: Read
            if (wdcVersion === 3) data.move(header.offsetMapIDCount * 4);
        ...
        for (
            ...
            if (header.tactKeyHash !== BigInt(0)) {
                ...
                if (
                    ...
                    wdcVersion === 3 &&
                    isNormal &&
                    (header.idListSize > 0 || header.copyTableCount > 0)
                ) {
                ...
                if (isZeroed) {
                    log.write(
                        "Skipping all-zero encrypted section " +
                            sectionIndex +
                            " in file " +
                            this.fileName
                    );
            ...
            for (let i = 0, n = header.recordCount; i < n; i++) {
                ...
                const recordEnd =
                    section.recordDataOfs + recordOfs + actualRecordSize;
                ...
                for (const [prop, type] of this.schema.entries()) {
                    ...
                    const fieldOffsetBytes = Math.floor(
                        recordFieldInfo.fieldOffsetBits / 8
                    );
                    ...
                    switch (recordFieldInfo.fieldCompression) {
                            ...
                            switch (fieldType) {
                                case FieldType.String:
                                    if (isNormal) {
                                        ...
                                        out[prop] = data.readString(
                                            data.indexOf(0x0) - data.offset,
                                            "utf8"
                                        );
                                ...
                                case FieldType.Int8:
                                    out[prop] = data.readInt8(count);
                                    break;
                                case FieldType.UInt8:
                                    out[prop] = data.readUInt8(count);
                                    break;
                                case FieldType.Int16:
                                    out[prop] = data.readInt16LE(count);
                                    break;
                                case FieldType.UInt16:
                                    out[prop] = data.readUInt16LE(count);
                                    break;
                                case FieldType.Int32:
                                    out[prop] = data.readInt32LE(count);
                                    break;
                                case FieldType.UInt32:
                                    out[prop] = data.readUInt32LE(count);
                                    break;
                                case FieldType.Int64:
                                    out[prop] = data.readInt64LE(count);
                                    break;
                                case FieldType.UInt64:
                                    out[prop] = data.readUInt64LE(count);
                                    break;
                                case FieldType.Float:
                                    out[prop] = data.readFloatLE(count);
                                    break;
                            }
                            break;
                        ...
                        case CompressionType.CommonData:
                            if (commonData[fieldIndex].has(recordID))
                                out[prop] =
                                    commonData[fieldIndex].get(recordID);
                            else
                                out[prop] =
                                    recordFieldInfo.fieldCompressionPacking[0]; // Default value
                            break;
                        ...
                        case CompressionType.BitpackedIndexedArray:
                            ...
                            const bitOffset = BigInt(
                                recordFieldInfo.fieldOffsetBits & 7
                            );
                            const bitSize =
                                1n << BigInt(recordFieldInfo.fieldSizeBits);
                            const bitpackedValue =
                                (rawValue >> bitOffset) & (bitSize - BigInt(1));

                            if (
                                recordFieldInfo.fieldCompression ===
                                CompressionType.BitpackedIndexedArray
                            ) {
                                out[prop] = new Array(
                                    recordFieldInfo.fieldCompressionPacking[2]
                                );
                                for (
                                    let i = 0;
                                    i <
                                ...
                                if (bitpackedValue in palletData[fieldIndex])
                                    out[prop] =
                                        palletData[fieldIndex][bitpackedValue];
                                else
                                    throw new Error(
                                        "Encountered missing pallet data entry for key " +
                                            bitpackedValue +
                                            ", field " +
                                            fieldIndex
                                    );
                    ...
                    if (
                        ...
                        if (!Array.isArray(type)) {
                            ...
                            else castBuffer.writeBigUInt64LE(BigInt(out[prop]));

                            castBuffer.seek(0);
                            switch (fieldType) {
                                case FieldType.String:
                                    throw new Error(
                                        "Compressed string arrays currently not used/supported."
                                    );

                                case FieldType.Int8:
                                    out[prop] = castBuffer.readInt8();
                                    break;
                                case FieldType.UInt8:
                                    out[prop] = castBuffer.readUInt8();
                                    break;
                                case FieldType.Int16:
                                    out[prop] = castBuffer.readInt16LE();
                                    break;
                                case FieldType.UInt16:
                                    out[prop] = castBuffer.readUInt16LE();
                                    break;
                                case FieldType.Int32:
                                    out[prop] = castBuffer.readInt32LE();
                                    break;
                                case FieldType.UInt32:
                                    out[prop] = castBuffer.readUInt32LE();
                                    break;
                                case FieldType.Int64:
                                    out[prop] = castBuffer.readInt64LE();
                                    break;
                                case FieldType.UInt64:
                                    out[prop] = castBuffer.readUInt64LE();
                                    break;
                                case FieldType.Float:
                                    out[prop] = castBuffer.readFloatLE();
                                    break;
                            }
                        } else {
                            for (
                                let i = 0;
                                i < recordFieldInfo.fieldCompressionPacking[2];
                                i++
                            ) {
                                castBuffer.seek(0);
                                if (out[prop] < 0)
                                    castBuffer.writeBigInt64LE(
                                        BigInt(out[prop][i])
                                    );
                                else
                                    castBuffer.writeBigUInt64LE(
                                        BigInt(out[prop][i])
                                    );
                                ...
                                switch (fieldType) {
                                    ...
                                    case FieldType.Int8:
                                        out[prop][i] = castBuffer.readInt8();
                                        break;
                                    case FieldType.UInt8:
                                        out[prop][i] = castBuffer.readUInt8();
                                        break;
                                    case FieldType.Int16:
                                        out[prop][i] = castBuffer.readInt16LE();
                                        break;
                                    case FieldType.UInt16:
                                        out[prop][i] =
                                            castBuffer.readUInt16LE();
                                        break;
                                    case FieldType.Int32:
                                        out[prop][i] = castBuffer.readInt32LE();
                                        break;
                                    case FieldType.UInt32:
                                        out[prop][i] =
                                            castBuffer.readUInt32LE();
                                        break;
                                    case FieldType.Int64:
                                        out[prop][i] = castBuffer.readInt64LE();
                                        break;
                                    case FieldType.UInt64:
                                        out[prop][i] =
                                            castBuffer.readUInt64LE();
                                        break;
                                    case FieldType.Float:
                                        out[prop][i] = castBuffer.readFloatLE();
                                        break;
                                }
                            }
                        }
