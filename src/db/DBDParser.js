...
const PATTERN_COLUMN =
    /^(int|float|locstring|string)(<[^:]+::[^>]+>)?\s([^\s]+)/;
    ...
    parseChunk(chunk) {
        if (chunk[0] === "COLUMNS") {
            ...
            for (const line of chunk) {
                ...
                if (commentMatch !== null) continue;

                // Layout hashes.
                const layoutMatch = line.match(PATTERN_LAYOUT);
                if (layoutMatch !== null) {
                    // LAYOUT 0E84A21C, 35353535
                    entry.addLayoutHashes(
                        ...layoutMatch[1].split(",").map((e) => e.trim())
                    );
                    continue;
                }
                ...
                if (fieldMatch !== null) {
                    ...
                    if (fieldType === undefined)
                        throw new Error(
                            "Invalid DBD: No field type defined for " +
                                fieldName
                        );
                    ...
                    if (fieldMatch[5]?.length > 0) field.isSigned = false;

                    // Parse data size (eg '32').
                    if (fieldMatch[6] !== undefined) {
                        const dataSize = parseInt(fieldMatch[6]);
                        if (!isNaN(dataSize)) field.size = dataSize;
                    }
